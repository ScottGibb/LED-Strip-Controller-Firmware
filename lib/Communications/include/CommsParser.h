...
/**
 * @brief CommsParser class concerned with byte level parsing and updating the
...
public:
   ...
   */
  CommsParser(vector<ICommunicator *> comms, const uint32_t ledTxRate,
              const uint32_t pwrTxRate);
  /**
...
private:
  ...
  void parseAndUpdate();
  /**
   * @brief Soft timed method that will send out periodic updates of what the
   * LEDs are currently doing
   *
   */
  void sendLEDUpdate();
  /**
