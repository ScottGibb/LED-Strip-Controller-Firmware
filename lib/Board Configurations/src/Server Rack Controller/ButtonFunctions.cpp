...
const uint32_t buttonPins[4] = {BUTTON_1_PIN, BUTTON_2_PIN, BUTTON_3_PIN,
                                BUTTON_4_PIN};
const func_type functions[4] = {buttonOneFunction, buttonTwoFunction,
                                buttonThreeFunction, buttonFourFunction};

// Internal Function Prototypes
static void toggleButtonLogic(uint8_t *brightness, FadeState_t *state,
                              RGBColourDriver *colDriver, HueDriver *hueDriver,
 ...
 */
void buttonOneFunction() {
  toggleButtonLogic(&prevBrightness[0], &prevStates[0], stripDrivers[0],
                    hueDrivers[0], fadeDrivers[0]);
}
...
void buttonTwoFunction() {
  ...
  toggleButtonLogic(&prevBrightness[1], &prevStates[1], stripDrivers[1],
                    hueDrivers[1], fadeDrivers[1]);
}
...
void buttonThreeFunction() {
  ...
  toggleButtonLogic(&prevBrightness[2], &prevStates[2], stripDrivers[2],
                    hueDrivers[2], fadeDrivers[2]);
}
...
/**
 * @brief Turns on and off the selected channels LED, pauses the Fade loop as
 * well.
 *
 ...
 */
void toggleButtonLogic(uint8_t *brightness, FadeState_t *state,
                       RGBColourDriver *colDriver, HueDriver *hueDriver,
                       FadeDriver *fadeDriver) {
