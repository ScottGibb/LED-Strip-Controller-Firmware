 ...
 * @brief Fade Driver Object Implementation code, outlining all the fade Driver
 * works.
 * @version 0.1
 * @date 2022-10-03
 *
...
void FadeDriver::startFade(FadeState_t fadeState) { currentState = fadeState; }
...
void FadeDriver::fadeLoop() {
  if ((millis() - lastFadeUpdateTime > STEP_SIZE)) {
    ...
    case COLOUR_CHANGE:
      sineWave(); // TODO: Add Future Capability to swap out which waveform its
                  // colour changing
      break;
...
FadeState_t FadeDriver::getFade() { return currentState; }
...
void FadeDriver::sawToothWave() {
  uint8_t brightness =
      static_cast<uint8_t>((((currentStep) * 1.0) / currentState.period) *
                           currentState.maxBrightness);
...
void FadeDriver::triangleWave() {
  ...
  if (currentStep < currentState.halfPeriod) {
    brightness =
        static_cast<uint8_t>((((currentStep) * 1.0) / currentState.halfPeriod) *
                             currentState.maxBrightness);
  } else {
    brightness = (((currentState.period - currentStep) * 1.0) /
                  currentState.halfPeriod) *
                 currentState.maxBrightness;
  }
  driver->setBrightness(brightness);
}
...
void FadeDriver::sineWave() {
  ...
  uint8_t brightness =
      static_cast<uint8_t>((currentState.maxBrightness / 2) * sin(angle * PI) +
                           (currentState.maxBrightness / 2.0));
  driver->setBrightness(brightness);
}