...
MEMORY_ERR MemoryHandler::saveData(SEGMENT seg, uint16_t pos, uint8_t *data,
                                   uint16_t dataLen) {
  uint16_t saveAddress =
      MEMORY_MAP[seg].MEMORY_START + MEMORY_MAP[seg].SLOT_SIZE * pos;
...
MEMORY_ERR MemoryHandler::loadData(SEGMENT seg, uint16_t pos, uint8_t *data,
                                   uint16_t dataLen) {
...
MEMORY_ERR MemoryHandler ::checkValidity(SEGMENT seg, uint16_t pos,
                                         uint16_t dataLen) {
...
MemoryHandler *
MemoryHandler::getInstance(const std::map<SEGMENT, MemoryMap_t> memory) {
