...
UserModeSelector::UserModeSelector(LEDDriver *driver) : driver(driver) {
  controlFlag = false;
  handler = nullptr;
  mode = MEMORY_MODE::NOT_SELECTED;
}
UserModeSelector::~UserModeSelector() { delete handler; }
...
void UserModeSelector::stop() { controlFlag = false; }

void UserModeSelector::setMode(MEMORY_MODE memMode, uint8_t *buffer,
                               uint32_t length) {
