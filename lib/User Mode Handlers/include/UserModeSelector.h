...
/**
 * @brief MEMORY_MODE enum outlining the different ways the memory for user mode
 * can be interpreted
 *
 */
...
/**
 * @brief UserModeSelector class responsible for calling the apropriate handler
 * at a time to interpret the memory block and subsequently make the correct
...
public:
  ...
  void loop();
  /**
   * @brief Starts the user mode sequence, the user must call setMode before
   * this for the system to work
   *
  ...
  void stop();
  /**
   * @brief Sets the mode that the channel will then display the pattern
   * outlined in the buffer
   *
