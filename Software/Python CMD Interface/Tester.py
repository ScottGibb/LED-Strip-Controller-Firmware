...
while True:
    ...
    if mode == FADE_TYPE.HUE_CTRL.value:
        ...
        saturation = int(input("Please enter the Saturation value (0.0-1.0)"))
        brightness = int(input("Please enter the Brightness value (0.0-1.0)"))
        hue_bytes = hue.to_bytes(2, 'big')
        tx_msg = bytearray([channel, mode]) + hue_bytes + bytearray([saturation, brightness])
    ...
    # Fill Array to standard message size
    padding = TX_MSG_SIZE - len(tx_msg)
    print(len(tx_msg))
    if padding > 0:
        tx_msg = tx_msg + bytearray([0] * padding)
    ser.write(tx_msg)
    print("Sent Message")
